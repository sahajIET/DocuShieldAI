...
def post_process_and_validate_entities(entities, full_text_by_page, nlp_model):
    """
    An upgraded, stricter validation filter to aggressively remove false positives.
    """
    validated_entities = []
    processed_spans = set()
    ...
    for entity in sorted(entities, key=lambda x: len(x['text']), reverse=True):
        ...
        # --- RULE 0: PRE-FILTERING (Noise Reduction) ---
        # A. Filter out very short, non-numeric strings which are often OCR errors or irrelevant.
        if len(text) <= 2 and not text.isdigit():
            logging.info(f"FILTERED (Too Short): '{text}' ({label})")
            continue
        
        # B. Filter out terms from our hard-coded deny list.
        if text.lower() in DENY_LIST_TERMS:
            logging.info(f"FILTERED (Deny List): '{text}' ({label})")
            continue

        # --- RULE 1: HIGH-CONFIDENCE PASS-THROUGH ---
        # Regex matches and high-score Presidio matches are trusted.
        high_confidence_labels = {"REGEX_PAN_ID", "REGEX_AADHAAR_ID", "REGEX_EMAIL_ADDRESS"}
        if label in high_confidence_labels:
            logging.info(f"VALIDATED (High Confidence Label): '{text}' ({label})")
            validated_entities.append(entity)
            processed_spans.add(entity_span)
            continue
            
        # --- RULE 2: PRESIDIO SCORE THRESHOLD ---
        # Set a reasonable confidence threshold for Presidio results.
        # This is the KEY FIX for issues like 'xe'.
        # START with 0.5 and adjust based on your documents.
        PRESIDIO_THRESHOLD = 0.0 
        if 'PRESIDIO' in label:
            score = entity.get('score', 0)
            if score < PRESIDIO_THRESHOLD:
                logging.info(f"FILTERED (Low Presidio Score: {score:.2f}): '{text}' ({label})")
                continue
            # If the score is high enough, we can often trust it directly
            if score >= 0.85:
                 logging.info(f"VALIDATED (High Presidio Score: {score:.2f}): '{text}' ({label})")
                 validated_entities.append(entity)
                 processed_spans.add(entity_span)
                 continue

        # --- RULE 3: CONTEXTUAL AND SEMANTIC CHECKS (For remaining ambiguous entities) ---
        # This now applies to medium-confidence Presidio results.
        ...
        if label in ambiguous_labels:
            if is_context_relevant(full_text, entity['start_char'], entity['end_char']):
                logging.info(f"VALIDATED (Context Keyword): '{text}' ({label})")
            else:
                # Only run expensive semantic check if context is ambiguous
                semantic_score = get_semantic_pii_score(full_text, entity['start_char'], entity['end_char'], nlp_model)
                SEMANTIC_THRESHOLD = 0.40 # A good starting point
                if semantic_score > SEMANTIC_THRESHOLD:
                    logging.info(f"VALIDATED (Semantic Score: {semantic_score:.2f}): '{text}' ({label})")
                else:
                    logging.info(f"FILTERED (Low Context/Semantics: {semantic_score:.2f}): '{text}' ({label})")
...
if __name__ == "__main__":
    ...
    input_pdf_path = "12th.pdf" # Make sure to use your input file
    output_pdf_path = "redacted_final.pdf"
    ...
    if not os.path.exists(input_pdf_path):
        ...
        if all_pages_data:
            ...
            # --- 3. DETECT POTENTIAL ENTITIES (Presidio + Custom Regex) ---
            logging.info("\n--- Phase 1: Detecting All Potential Entities ---")
            for page_data in all_pages_data:
                # Use Presidio as the primary, robust detection engine
                ...
                # Use our refined regex for highly specific, trusted patterns
            ...
            if validated_entities:
                # ... (rest of your redaction and logging code is perfect and remains the same) ...
                ...
                # Make sure you've implemented the 'quads=True' suggestion in redact_sensitive_info for best results
